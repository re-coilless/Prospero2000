...
function OnModInit()
	...
	--magic numbers
	local numbers_res = "mods/p2k/files/scripts/magic_numbers_resolution.xml"
	if( ModSettingGetNextValue( "p2k.USE_CUSTOM_RES" )) then
		local custom_res = D_extractor( ModSettingGetNextValue( "p2k.CUSTOM_RES" ), true )
		ModTextFileSetContent( numbers_res, string.gsub( string.gsub( ModTextFileGetContent( numbers_res ), "X=\"427\"", "X=\""..custom_res[1].."\"" ), "Y=\"242\"", "Y=\""..custom_res[2].."\"" ))
	end
	if( not( ModSettingGetNextValue( "p2k.NO_MAGIC" ))) then
		ModMagicNumbersFileAdd( numbers_res )
	end
	
	--shaders
	if( not( ModSettingGetNextValue( "p2k.NO_SHADER_STUFF" ))) then
		local shader_old = "data/shaders/post_final.frag"
		local shader_custom_base = "mods/p2k/files/scripts/shader"
	...
	--custom font
	if( not( ModSettingGetNextValue( "p2k.NO_FONT" ))) then
		local path = "data/fonts/font_pixel_"
		ModTextFileSetContent( path.."noshadow.xml", ModTextFileGetContent( path.."proper.xml" ))
	end
end

--nothing is fucking cashed
--and code is disgusting
--i will fix, i swear
function OnWorldPreUpdate()
	...
	if( not( GameIsInventoryOpen())) then
		...
		dofile( "mods/p2k/files/scripts/extensions.lua" )
		...
		if( ModSettingGetNextValue( "p2k.P2K_ENABLED" )) then
			...
			if( help_tips == nil ) then
				...
				if( GlobalsGetValue( "HERMES_IS_REAL", "0" ) == "1" ) then
					local hermes_tips = {
						"Did you know that all the known holy scriptures were designed by HermeS, causing "..( math.floor( frame_num*7.67 )%8 + 1 ).."e"..( math.floor( frame_num*8.867 )%8 + 1 ).." casualties weekly?",
						"Projections showed HermeS holding over 103% of market share by 1950, a forecast demolished by the Prospero Inc.",
						"Evaluation of HermeS brand has dropped by 11 points since Prospero has entered the market.",
						"Propero2000 has over "..( frame_num%9 + 0.1*( math.floor( frame_num*1.7 )%6 + 1 )).." trillion verified human users - a "..( 0.19*( math.floor( frame_num*4.83 )%10 + 1 )).." percent gain over 19A.",
						"HermeS has been repeatedly sued for \"reality fraud and market alterations\" ("..math.floor( 17.386*( math.floor( frame_num*0.63 )%40 + 1 )).." cases are pending).",
						"HermeS mostly employs a tectonic technique of Shape by Contradiction of which there're many refutations.",
						"Out of "..math.floor( 1.12*( math.floor( frame_num*3.9 )%9 + 1 )).." of the latest HermeS releases only "..( math.floor( frame_num*8.7 )%5 + 1 ).." are considered Divine Heritage.",
						"HermeS ranks 7.8-2.7 positions below the Prospero Inc. at the closing quarter of The Enumeration.",
						"H.E.Fosdick has never used 19A and never plans to, disproving official Declaration of Authenticity.",
					}
					for i,tip in ipairs( hermes_tips ) do
						table.insert( help_tips, tip )
					end
				end
			end
...
function OnPlayerSpawned( hooman )
	...
	if( not( ModSettingGetNextValue( "p2k.SILENT_BOOT" ))) then
		local c_x, c_y = GameGetCameraPos()
		GamePlaySound( "mods/p2k/files/sfx/p2k.bank", "bootup", c_x, c_y )
	end
end